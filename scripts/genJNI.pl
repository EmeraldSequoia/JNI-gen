...
use esfiles;   # For file comparison, rename, etc
...
sub dieWithError {
    my $error = shift;
    $error =~ s/\n$//o;
    die "$fullInputPath:$lineNumber: error: $error\n";
}
...
$classWithSpacesForHeader getRetainedCopy(JNIEnv *jniEnv) const;
...
EOF
    ...
    foreach my $castClass (@{$cls->{castoks}}) {
        ...
        print HEADER "$castClassWithSpacesForHeader &$castMethod() const;\n";
    }
...
$cls->{esClass}::initAndRetain(JNIEnv *jniEnv) {
    ...
    _javaClass = jniEnv->FindClass("$cls->{javaPath}");
    ESAssert(_javaClass);
...
EOF
    ...
    if (!$methodIDField) {
        $methodIDField = jniEnv->$getter(_javaClass, "$method->{name}", "$method->{descriptor}");
...
EOF
        ...
        if ($returnType eq "void") {
            print CPP "    jniEnv->$method->{callFunction}($object, $methodIDField$callParams);\n";
        } else {
            print CPP "    return $returnType(jniEnv->$method->{callFunction}($object, $methodIDField$callParams));\n";
        }
        print CPP "}\n";
    }
...
EOF
    ...
    foreach my $field (@{$cls->{fields}}) {
        ...
        print HEADER (" " x $numSpaces);
        print HEADER "$fieldGetter(JNIEnv *jniEnv) const;\n";
        $numSpaces = 20;   # 28 - 4 - 4
        if ($field->{isStatic}) {
            $numSpaces = 13;
        }
        ...
        print CPP "$cls->{esClass}::$fieldGetter(JNIEnv *jniEnv) const {\n";
        my $fieldIDField = "_fieldID$fieldIDCount" . "_$field->{name}";
        my $getter;
        if ($field->{isStatic}) {
...
while (<FILE>) {
    ...
    if (/^\s*class\s*(\S+)\s*$/o) {
        ...
        my $castClass = ESNameForJavaPath $1;
        push @{$currentClass->{ISAs}}, $castClass;
        checkFriend($castClass, $currentClass->{esClass});
    } elsif (/\S/) {
